...
<main class="main">
    ...
    <section class="content attendance-correction">
        <!-- 정정신청 폼 -->
        <form action="${pageContext.request.contextPath}/updateClass.cl" method="get">
            <div class="card correction-form">
                <h2>반 등록</h2>
                <input type="hidden" name="classNo" value="${classInfo[0].classNo}"/>
                <div class="form-group">
                    <label>반 이름</label>
                    <input type="text" name="className" value="${classInfo[0].className}" readonly/>
                </div>

                <div class="form-row">
                    <div class="form-col">
                        <label>시작 날짜</label>
                        <input type="date" name="startDate" class="start-date-input" value="${classInfo[0].startDate}" onchange="dateSet()"/>
                    </div>
                    <div class="form-col">
                        <label>종료 날짜</label>
                        <input type="date" name="endDate" class="end-date-input" value="${classInfo[0].endDate}" onchange="dateSet()"/>
                    </div>
                </div>
            ...
            <div class="card">
                <table>
                    ...
                    <tbody class="lecture-tbody">
                        <c:forEach var="lecList" items="${lectureInfo}">
                            <tr>
                                <td class="check-box"><input type="checkbox" name="noCheck"/></td>
                                <td class="lecture-no"><input type="text" name="lectureNoList" value="${lecList.lectureNo}"/></td>
                                <td>${lecList.lectureName}</td>
    ...
    function selectLecture() {
        ...
        lectureList.forEach(function (data,index) {
            ...
            if (selectedLectureVal == data.lectureNo) {
                ...
                tr.innerHTML = '<tr>'
                                    +'<td class="check-box"><input type="checkbox" name="noCheck"/></td>'
                                    +'<td class="lecture-no"><input type="text" name="lectureNoList" value="'+data.lectureNo+'"></inputclass></td>'
